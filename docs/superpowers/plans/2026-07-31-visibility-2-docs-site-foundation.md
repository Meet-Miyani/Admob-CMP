...
**Status:** Repository implementation complete. External Cloudflare and Search
Console operational state remains owner-controlled and is tracked separately.

**Design reconciliation:** The visual implementation originally described in
this plan was replaced by the implemented and verified
[`Documentation Native Reference Theme Plan`](./2026-08-01-docs-native-reference-theme.md).
That plan and its companion
[`design specification`](../specs/2026-08-01-docs-native-reference-theme-design.md)
are authoritative for typography, colors, code surfaces, tables, component
shape, generated social images, and motion. Historical Space Grotesk/Inter,
permanent-dark code, large-radius, shadow, decorative-label, and entrance-motion
snippets below must not be reapplied.

**Goal:** Stand up `docs-site/` — an Astro + Starlight documentation site at `https://ads.avinya.dev` with the Native Reference visual language, complete SEO plumbing, an AI-crawler-permissive `robots.txt`, `llms.txt`, build-time Mermaid SVG, a Dokka 2.2.0 API reference at `/api/`, and Cloudflare Pages deployment through the repository's release workflow.

**Architecture:** A single Astro project rooted at `docs-site/` inside this repo. Starlight owns routing, search (Pagefind), and theme switching; `docs-site/src/styles/tokens.css` maps Starlight onto a compact native-sans reference system with paired light/dark surfaces and restrained Avinya accent usage. SEO extras Starlight does not emit (OG images, JSON-LD) come from a `components.Head` override backed by pure functions in `docs-site/src/lib/seo.ts`, which are unit-tested with Vitest. The Kotlin API reference is generated by the Dokka Gradle plugin in the existing Gradle build and `Sync`-copied into `docs-site/public/api/` so Astro ships it verbatim. Deployment is owned by `.github/workflows/release.yml`; local verification remains in `scripts/release-readiness.sh`.

**Tech Stack:** Astro 7.1.6 · @astrojs/starlight 0.41.5 · @astrojs/markdown-remark 7.2.2 · @astrojs/sitemap 3.7.3 · starlight-llms-txt 0.11.0 · rehype-mermaid 3.0.0 + Playwright Chromium · astro-og-canvas 0.13.0 · @fontsource/noto-sans 5.3.0 · Vitest 4.1.10 · Dokka Gradle plugin 2.2.0 · Cloudflare Pages
...
- **Shared file-path contract — other plans depend on these EXACT paths. Do not rename them:**
  ...
  ```
- **`tokens.css` preserves the original 19 custom-property names and adds three Native Reference tokens:** `--admob-accent-text`, `--admob-reading`, and `--admob-ease`. Plans 4 and 5 consume the complete 22-token contract; never rename a token or introduce a parallel palette.
- **Native Reference invariants:** native UI typography for every non-code surface; JetBrains Mono for code; 16px/1.6 body with a 45rem reading measure; 32px desktop and 28px mobile H1; 24px H2; 19px H3; 13px sidebar/TOC; paired `github-light`/`github-dark` code; 6px radii; one-pixel neutral borders; no shadows or spatial motion.
- **Verification stays local.** `scripts/release-readiness.sh` Section 8 runs the Astro suite, build verification, `check:theme`, and `check:overflow`. Do not add or modify GitHub verification jobs.
...
| Path | Responsibility |
|---|---|
| `docs-site/package.json` | Pinned dependency set and the `dev` / `build` / `preview` / `test` / `verify` / `check:theme` / `check:overflow` scripts. |
...
| `docs-site/src/styles/tokens.css` | Native Reference typography and component overrides + the 22 `--admob-*` tokens + the `--sl-*` remapping. Consumed by Plans 3, 4, 5. |
...
| `docs-site/src/pages/og/[...route].ts` | Generates one 1200×630 OG card per docs page with bundled Noto Sans assets and no build-time font download. |
...
| `docs-site/public/fonts/*.woff2` | JetBrains Mono assets used by code. Legacy font files may remain on disk but are not loaded. |
...
| `docs-site/src/lib/rehype-table-scroll.mjs` | Accessible Markdown-table wrapper with keyboard-focusable regions and wide-table classification. |
| `docs-site/scripts/check-theme.mjs` | Native Reference computed-style gate using a freshly built isolated server. |
| `docs-site/scripts/check-overflow.mjs` | Playwright check: no horizontal overflow at 375 px on every built page, using its own isolated server. |
| `docs-site/test/seo.test.ts` | Vitest unit tests for `src/lib/seo.ts`. |
| `docs-site/test/rehype-table-scroll.test.ts` | Table-transform structure, accessibility, preservation, and double-wrap regression coverage. |
| `docs-site/test/og-theme.test.ts` | Bundled Noto Sans OG-font contract. |
...
| `.github/workflows/release.yml` | Existing deployment workflow. Do not add a separate docs verification workflow. |
| `scripts/release-readiness.sh` | Local release gate; Section 8 runs both docs browser checks after the Astro build. |
...
> **As-built note:** The package snippet below records the original scaffold.
> The current contract additionally includes `@astrojs/markdown-remark` 7.2.2,
> `@fontsource/noto-sans` 5.3.0, and `check:theme`. Use the checked-in
> `docs-site/package.json`; do not restore the historical dependency set.
...
### Task 2: Historical initial theme — superseded by Native Reference

> **Do not execute the visual snippets in this task.** They are retained only to
> explain the foundation's history. The complete replacement, including exact
> token values, typography, paired code surfaces, tables, controls, and motion,
> is the implemented
> [`Documentation Native Reference Theme Plan`](./2026-08-01-docs-native-reference-theme.md).
...
- Historical output: the original 19-token baseline. The current file preserves
  those names and adds `--admob-accent-text`, `--admob-reading`, and
  `--admob-ease`; Plans 4 and 5 consume the complete 22-token contract.
...
> **As-built visual correction:** Preserve the IA and SEO structure below, but
> use the current paired `github-light`/`github-dark` Expressive Code themes,
> 5.5:1 contrast correction, unified Markdown processor, native Mermaid font
> stack, and JetBrains Mono-only preload. Do not restore the historical display
> and body font preloads.
...
> **As-built visual correction:** OG rendering uses bundled Noto Sans 600/400
> assets from `@fontsource/noto-sans`. Do not restore Space Grotesk, Inter, remote
> Fontsource downloads, or build-time network font fetching. The SEO data and
> route contracts below remain valid.
...
> **As-built pipeline correction:** Configure
> `markdown.processor: unified({ rehypePlugins: [...] })` through
> `@astrojs/markdown-remark`. Run `rehype-mermaid` before
> `rehypeTableScroll`. Mermaid uses the native UI stack. The deprecated
> `markdown.rehypePlugins` and Inter configuration below are historical only.
...
### Task 9: Deployment integration and Cloudflare operations

**Status:** Repository integration is complete. External Cloudflare project,
domain, credentials, and live-host state are owner-controlled operational state.

**Files:**
- Existing: `.github/workflows/release.yml`
- Existing: `scripts/release-readiness.sh`
- Do not create: `.github/workflows/docs-site.yml`

**Interfaces:**
- `.github/workflows/release.yml` owns docs deployment on the repository's
  established release path.
- `scripts/release-readiness.sh` owns all local docs verification. GitHub
  workflows intentionally do not add SDK or docs verification jobs.
- Cloudflare Pages remains a Direct Upload project named `admob-cmp-docs` with
  canonical host `https://ads.avinya.dev`.

- [x] **Step 1: Preserve the existing deployment architecture**

Do not add a pull-request workflow, a parallel docs workflow, or a verification
job in front of publication. Any future workflow maintenance must follow the
root `AGENTS.md` policy and retain the single `release.yml` owner.

- [ ] **Step 2: Confirm owner-controlled Cloudflare state**

A human owner verifies in Cloudflare and GitHub settings that:

1. The `admob-cmp-docs` Direct Upload project exists.
2. `ads.avinya.dev` is attached as the custom domain.
3. `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` are configured with
   the minimum required scope.
4. The Pages preview host redirects to the canonical hostname.
5. Managed crawler restrictions do not override the repository's authored
   `robots.txt`.

These are external state checks, not agent-authorized provisioning or deployment
steps. An agent may inspect and report them only when explicitly asked.

- [ ] **Step 3: Verify the live host after an owner-authorized deployment**
...
Expected after deployment: the canonical host and `/api/` return 200, the
preview host redirects to `https://ads.avinya.dev/`, and no managed restrictive
`Disallow` is injected. No commit is produced by these operational checks.
...
Expected: valid XML naming `sitemap-0.xml`; at least `26`; `NO PREVIEW HOST IN SITEMAP OK`. The foundation baseline is 26 URLs; Plan 4's generated diagram-prose route raises it to 27. That last check is the direct guard against repeating the spec §5 defect.
...
### Task 11: Full-site local verification

**Status:** Implemented and integrated into release readiness.

**Files:**
- Existing: `docs-site/scripts/verify-build.mjs`
- Existing: `docs-site/scripts/check-theme.mjs`
- Existing: `docs-site/scripts/check-overflow.mjs`
- Existing: `scripts/release-readiness.sh`

**Interfaces:**
- `verify-build.mjs` validates sitemap, robots, llms, canonical, OG, JSON-LD,
  Pagefind, Mermaid, and generated API output.
- `check-theme.mjs` serves the fresh `dist/` tree on an isolated ephemeral
  `127.0.0.1` port unless `PREVIEW_URL` is explicit. It asserts Native
  Reference typography, paired syntax surfaces, syntax-color coverage, table
  accessibility, component shape, focus, theme switching, containment, and
  reduced-motion behavior.
- `check-overflow.mjs` independently serves the same fresh build on its own
  ephemeral port and checks every page at 375px in both themes.
- Section 8 of `scripts/release-readiness.sh` runs the docs suite and both
  browser gates after the Astro build.

- [x] **Step 1: Maintain the build verifier and focused unit coverage**

Keep the SEO tests, table-transform tests, and bundled OG-font test aligned with
the checked-in implementation. Do not duplicate generated artifacts or commit
`docs-site/dist/` or `docs-site/public/api/`.

- [x] **Step 2: Maintain isolated browser gates**

The browser scripts own server startup, readiness polling, cleanup, and unique
ports. Do not require a manually backgrounded `npm run preview`, do not bind a
fixed port by default, and do not allow one gate to reuse stale output from
another process.

- [x] **Step 3: Run the complete local chain**
...
Expected: all discovered tests pass, Astro reports the current authored/generated
page count, both isolated browser gates pass, `git diff --check` is clean, and
all eight readiness sections finish with `READINESS: PASS`. Do not add GitHub
verification jobs as a substitute for this local chain.

- [x] **Step 4: Report readiness before any integration action**

Report which readiness sections ran, which were skipped, and anything fixed.
Passing verification does not authorize a commit, push, PR, tag, release, or
external deployment; those remain separate owner decisions.

---

## Handoff notes for downstream plans

- **Visual authority:** All docs-site presentation inherits the implemented
  Native Reference design and complete 22-token contract. Do not restore the
  historical Task 2 values or introduce a parallel palette/type system.
- **Plan 3** owns the 24 authored guide routes and prose. Plan 4 may add the
  generated `/reference/diagrams-in-words/` route; Plan 5 owns the landing page.
- **Plan 4** replaces the current diagram stubs and extends the isolated theme and
  overflow gates for the internal gallery and representative docs pages.
- **Plan 5** replaces the body/frontmatter contract of
  `docs-site/src/content/docs/index.mdx` according to its exact Native Reference
  landing plan.
- **Plan 7** owns screenshot capture and manifest data. Plan 3 and Plan 5 own
  placement and consume assets only through `Screenshot.astro`.
- **Verification and deployment:** local verification remains in
  `scripts/release-readiness.sh`; deployment remains in
  `.github/workflows/release.yml`. Do not add a docs verification workflow.
- **Canonical host:** never point Astro `site:` at `*.pages.dev`.

## Self-review

This foundation plan is an as-built repository record with explicitly marked
historical snippets. Current source, root `AGENTS.md`, the Native Reference
design/plan, and local release-readiness behavior take precedence over any
remaining scaffold-era command sample. The handoff contract now preserves the
root-route IA, 22 shared tokens, paired syntax themes, accessible table pipeline,
bundled OG typography, isolated visual gates, generated `/api/` exception, and
single-workflow deployment policy without asking a downstream plan to recreate
